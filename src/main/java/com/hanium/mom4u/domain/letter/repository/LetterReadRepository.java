...
public interface LetterReadRepository extends JpaRepository<LetterRead, Long> {
    ...
    /** 특정 편지/회원의 읽음 엔티티 조회 */
    Optional<LetterRead> findByLetter_IdAndMember_Id(Long letterId, Long memberId);


    /** 편지 단건에 대해 '읽었는지' 여부 (readAt not null) */

    boolean existsByLetter_IdAndMember_IdAndReadAtIsNotNull(Long letterId, Long memberId);
